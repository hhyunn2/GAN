...
milestones_list = []
for i in range(10, num_epoch, 10):
    milestones_list.append(i)

train_df = pd.read_csv('/content/drive/My Drive/StarGAN/attribute.csv')
train_dataset = DataGenerator('/content/data/img_align_celeba', train_df, 'transform')
...
if __name__ == '__main__':
    ...
    model_G = Generator(5)
    model_D = Discriminator(5)

    model_G = model_G.to(device)
    model_D = model_D.to(device)
    
    optimizer_G = torch.optim.Adam(model_G.parameters(), lr = lr_G, betas = (0.5, 0.999))
    optimizer_D = torch.optim.Adam(model_D.parameters(), lr = lr_D, betas = (0.5, 0.999))
    scheduler_G = optim.lr_scheduler.MultiStepLR(optimizer_G, milestones = milestones_list, gamma = 0.9)
    scheduler_D = optim.lr_scheduler.MultiStepLR(optimizer_D, milestones = milestones_list, gamma = 0.9)
